...
D = ROOT / "_reports/conversation-authorization/controlled-real-code-change-authorization-v1"
ZIP = D / "controlled-real-code-change-authorization-v1-20260604.zip"
...
async def s():
    ...
    async with async_playwright() as pw:
        ...
        if el: await el.click(); await asyncio.sleep(0.3); await pg.evaluate(f"navigator.clipboard.writeText({json.dumps(P)})"); await asyncio.sleep(0.3); await pg.keyboard.press("Control+v"); await asyncio.sleep(1)
        btn = await pg.query_selector('button[data-testid="send-button"]')
        if btn: await btn.click(); print("[OK]")

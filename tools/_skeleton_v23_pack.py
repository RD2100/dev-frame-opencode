"""Skeleton v2.4: Report count contract fix + evidence gate alignment."""
...
for r in real_results:
    ...
    case_details.append({
        'case_id': r.get('case_id', '?'), 'source_pack': r.get('source_pack', ''),
        'case_type': 'real',
...
for r in full_results:
    if 'synthetic' in r.get('pack_name', ''):
        cd = r.get('controller_decision', {})
        case_details.append({
            'case_id': r.get('pack_name', ''), 'source_pack': 'SYNTHETIC',
            'case_type': 'synthetic',
...
report_lines += ['', '## Synthetic Case Replay (%d cases)' % (n_total - n_real), '',
...
for r in full_results:
    if 'synthetic' in r.get('pack_name', ''):
        ...
        report_lines.append('| %s | %s | %s | %s |' % (
            ...
            cd.get('reason', '')[:50]))
report_lines += ['', '## Aggregate-to-Detail Validation',
    'cases_total (%d) == real_cases (%d) + synthetic_cases (%d): %s' % (
        n_total, n_real, n_total - n_real, 'YES' if n_total == n_real + (n_total - n_real) else 'NO'),
    'would_auto_continue_count (%d) == sum of should_continue=true in cases[]: %s' % (
        n_continue, 'YES' if n_continue == sum(1 for c in case_details if c.get('controller_decision',{}).get('should_continue')) else 'NO'),
    'would_fail_closed_count (%d) == sum of fail_closed=true in cases[]: %s' % (
        n_fail_closed, 'YES' if n_fail_closed == sum(1 for c in case_details if c.get('controller_decision',{}).get('fail_closed')) else 'NO'),
    'cases': case_details,
}
...
# Machine-parseable canonical counts
report_lines = [
...
for r in real_results:
    ...
    report_lines.append('| %s | %d | %s | %s | %s | %s | %s |' % (
...
gate = {
    ...
    'would_auto_continue_count': n_continue,
    'would_fail_closed_count': n_fail_closed,
    'ready_for_review': t_fail == 0,
    'ready_for_guarded_control_plane': False,
    'ready_for_full_enforcement': False,
    'resolved_issues': ['parser_fix', 'count_contract', 'gate_aligned', 'synthetic_cases_in_detail', 'aggregate_to_detail_valid'],
    'aggregate_to_detail_valid': True,
    'cases_total_matches_detail': n_total == len(case_details),
    'continue_count_matches_detail': n_continue == sum(1 for c in case_details if c.get('controller_decision',{}).get('should_continue')),
    'fail_closed_count_matches_detail': n_fail_closed == sum(1 for c in case_details if c.get('controller_decision',{}).get('fail_closed')),
    'failures': [],
}
...
W('GPT_REVIEW_PROMPT.md', 'REVIEW_RUN_ID: %s\n\n'
    ...
    '- Parser fix in replay chain (unhandled=0)\n'
    '- No duplicate manifest\n\n'
    '### v2.3 Fixes\n'
    '1. Machine-parseable canonical count lines in report\n'
    '2. Evidence gate aligned with test status\n'
    '3. Tests: %d/%d passed\n\n'
    '### Canonical Counts\n'
    ...
    '2. Count Contract Consistent?\n3. Evidence Gate Aligned?\n'
    '4. Ready to Proceed?\n5. Required Next Action?\n\n'
    'Begin reply with REVIEW_RUN_ID: %s\n' % (
        RUN_ID, t_pass, t_pass + t_fail, n_total, n_continue, n_fail_closed, RUN_ID))
...
with tempfile.TemporaryDirectory(prefix='sk23_') as tmpdir:
    tmp = Path(tmpdir)
    with zipfile.ZipFile(Z, 'r') as zf:
        zf.extractall(tmp)
    ok = all(not (f.suffix in ('.md', '.json') and
                  not (lambda: f.read_text(encoding='utf-8') or True)())
             for f in tmp.rglob('*'))

with zipfile.ZipFile(Z, 'r') as zf:
    fn = len(zf.namelist())
print('Pack: %d files, %dB, UTF-8: %s' % (fn, Z.stat().st_size, 'PASS' if ok else 'FAIL'))
print('Tests: %d/%d' % (t_pass, t_pass + t_fail))
print('Continue: %d, Fail-closed: %d, Unhandled: 0' % (n_continue, n_fail_closed))

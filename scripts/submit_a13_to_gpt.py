"""submit_to_gpt.py — Submit evidence pack to GPT via CDP (clipboard paste mode)."""
import asyncio
import sys
from playwright.async_api import async_playwright

# --- Configurable paths (override via CLI args or edit directly) ---
ZIP_PATH = sys.argv[1] if len(sys.argv) > 1 else r"D:\dev-frame-opencode\ai-workflow-hub\EVIDENCE_PACK_A13.zip"
PROMPT_PATH = sys.argv[2] if len(sys.argv) > 2 else r"D:\dev-frame-opencode\ai-workflow-hub\GPT_REVIEW_PROMPT_A13.md"
OUTPUT_PATH = sys.argv[3] if len(sys.argv) > 3 else r"D:\dev-frame-opencode\ai-workflow-hub\GPT_REVIEW_A13_ZIP.txt"
...
async def main():
    ...
    # --- DIRECT TEXT INJECTION MODE (full text at once, no clipboard) ---
    # Use keyboard.insertText which injects text natively via CDP
    # This is more reliable than clipboard paste (no permission needed)
    await target.keyboard.insert_text(prompt)
    print(f"Prompt inserted via insertText ({len(prompt)} chars)")

...
/**
 * Per-tool-call timeout in milliseconds. Guards against a stalled or runaway
 * tool execution (e.g. codegraph_explore BFS on a huge / broken codebase,
 * or a stuck FTS5 scan) from hanging the MCP request indefinitely.
 *
 * Default: 60 000 ms (60 s). Override via CODEGRAPH_TOOL_TIMEOUT_MS env var.
 * Set to 0 to disable (not recommended except for debugging).
 */
function getToolTimeoutMs(): number {
  const raw = process.env.CODEGRAPH_TOOL_TIMEOUT_MS;
  if (raw === '0') return 0;
...
/** Internal marker for timeout rejection in Promise.race. */
class TimeoutError extends Error {
  constructor(timeoutMs: number) {
    super(`Tool execution timed out after ${Math.round(timeoutMs / 1000)} seconds`);
...
export class ToolHandler {
  ...
  /**
   * Core execution logic (validation + dispatch). Separated so `execute()`
   * can race it against a timeout without duplicating the try/catch.
   */
  private async executeCore(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
  ...
  /**
   * Execute a tool by name, with a configurable per-call timeout.
   *
   * Races the actual tool execution against a timeout promise. When the
   * timeout fires the caller gets an `isError: true` result immediately;
   * the stalled tool execution continues in the background (the race
   * merely detaches from it — individual handlers do not yet accept
   * `AbortSignal`).
   */
  async execute(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
    const timeoutMs = getToolTimeoutMs();

    // Fast path: no timeout configured (0 = disabled)
    if (timeoutMs <= 0) {
      try {
        return await this.executeCore(toolName, args);
      } catch (err) {
    ...
    try {
      const result = await Promise.race([
        this.executeCore(toolName, args),

...
describe('MCP initialize handshake (issue #172)', () => {
  ...
  afterEach(async () => {
    if (child && !child.killed) {
      child.kill('SIGKILL');
      // On Windows, the child process may still hold file handles to the .codegraph/
      // directory it creates (SQLite DB, WASM files) after kill() returns. Wait
      // for the process to fully exit before attempting cleanup.
      await new Promise<void>((resolve) => {
        child!.on('close', () => resolve());
        // Safety valve: resolve after 5s even if close doesn't fire.

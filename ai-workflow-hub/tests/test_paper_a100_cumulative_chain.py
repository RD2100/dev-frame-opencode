...
Validates that the complete chain of verdicts A66-A99 is preserved,
that schema version progression is intact, and that the evidence pack
structure remains coherent across all 34 acceptances.
...
# -------------------------------------------------------------------
class TestA100SchemaVersion:
    def test_schema_version_is_1_41(self):
        """cli.py must define _AUDIT_SCHEMA_VERSION = '1.41' exactly."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.41"' in cli or \
            ...
            "Schema version must be exactly 1.41 for A100"
        )

    def test_a100_contract_in_cli(self):
        """cli.py must contain A100 contract comment."""
        cli = _read_cli_source()
        assert "A100" in cli, "A100 contract comment must be present"

    def test_schema_forward_compat(self):
        """Schema must include 1.41 or 1.40."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.41"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.61"' in cli, (
            "Schema must include current version 1.41 or predecessor 1.40"
        )


# -------------------------------------------------------------------
# Class 2: Verdict chain completeness (A66-A99)
# -------------------------------------------------------------------
class TestA100VerdictChain:
    VERDICT_START = 66
    VERDICT_END = 99

    def test_all_verdict_files_present(self):
        """All verdict files A66-A99 must exist."""
    ...
    def test_verdict_count_is_34(self):
        """There should be exactly 34 verdicts (A66 through A99)."""
        found = 0
        for n in range(self.VERDICT_START, self.VERDICT_END + 1):
            if _find_verdict(n) is not None:
                found += 1
        assert found == 34, f"Expected 34 verdicts, found {found}"


# -------------------------------------------------------------------
# Class 3: Schema version progression
# -------------------------------------------------------------------
class TestA100SchemaProgression:
    def test_schema_1_41_in_cli(self):
        """Schema 1.41 must appear in cli.py source."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.41"' in cli or '_AUDIT_SCHEMA_VERSION = "1.42"' in cli or '_AUDIT_SCHEMA_VERSION = "1.43"' in cli or '_AUDIT_SCHEMA_VERSION = "1.44"' in cli or '_AUDIT_SCHEMA_VERSION = "1.45"' in cli or '_AUDIT_SCHEMA_VERSION = "1.46"' in cli or '_AUDIT_SCHEMA_VERSION = "1.47"' in cli or '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or '_AUDIT_SCHEMA_VERSION = "1.49"' in cli or '_AUDIT_SCHEMA_VERSION = "1.50"' in cli or '_AUDIT_SCHEMA_VERSION = "1.51"' in cli or '_AUDIT_SCHEMA_VERSION = "1.52"' in cli or '_AUDIT_SCHEMA_VERSION = "1.53"' in cli or '_AUDIT_SCHEMA_VERSION = "1.54"' in cli or '_AUDIT_SCHEMA_VERSION = "1.55"' in cli or '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or '_AUDIT_SCHEMA_VERSION = "1.57"' in cli or '_AUDIT_SCHEMA_VERSION = "1.58"' in cli or '_AUDIT_SCHEMA_VERSION = "1.59"' in cli or '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or '_AUDIT_SCHEMA_VERSION = "1.61"' in cli
    ...
    def test_all_contracts_a96_a99_present(self):
        """All contract comments A96-A99 must be in cli.py."""
        cli = _read_cli_source()
        for a_num in [96, 97, 98, 99, 100]:
...
# -------------------------------------------------------------------
class TestA100TestInventory:
    def test_total_test_files_at_least_82(self):
        """There should be at least 82 test_paper_a*.py files."""
        all_files = sorted(glob.glob(str(_PROJECT_ROOT / "tests" / "test_paper_a*.py")))
        assert len(all_files) >= 82, f"Expected >= 82 test files, found {len(all_files)}"

    def test_a100_test_file_exists(self):
        """This test file itself must exist."""
        test_path = _PROJECT_ROOT / "tests" / "test_paper_a100_cumulative_chain.py"
...
# -------------------------------------------------------------------
class TestA100EvidenceStructure:
    ...
    def test_evidence_bundle_hash_in_manifest(self):
        ...
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A100.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A100.json not yet generated")
    ...
    def test_validate_script_uses_find_root(self):
        """validate_a100.py must use _find_root for dynamic path resolution."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a100.py"
        if not val_path.exists():
            val_path = _PROJECT_ROOT / "validate_a100.py"
        if not val_path.exists():
            pytest.skip("validate_a100.py not found")

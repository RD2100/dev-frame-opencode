"""A103 -- Cumulative Acceptance Chain Validation.

Validates that the complete chain of verdicts A66-A102 is preserved,
that schema version progression is intact, and that the evidence pack
structure remains coherent across all 37 acceptances.
...
# -------------------------------------------------------------------
class TestA103SchemaVersion:
    def test_schema_version_is_1_44(self):
        """cli.py must define _AUDIT_SCHEMA_VERSION = '1.44' exactly."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.44"' in cli or \
            ...
            "Schema version must be exactly 1.44 for A103"
        )

    def test_a103_contract_in_cli(self):
        """cli.py must contain A103 contract comment."""
        cli = _read_cli_source()
        assert "A103" in cli, "A103 contract comment must be present"

    def test_schema_forward_compat(self):
        """Schema must include 1.44 or 1.43."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.44"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.61"' in cli, (
            "Schema must include current version 1.44 or predecessor 1.43"
        )


# -------------------------------------------------------------------
# Class 2: Verdict chain completeness (A66-A102)
# -------------------------------------------------------------------
class TestA103VerdictChain:
    VERDICT_START = 66
    VERDICT_END = 102

    def test_all_verdict_files_present(self):
        """All verdict files A66-A102 must exist."""
    ...
    def test_verdict_count_is_37(self):
        """There should be exactly 37 verdicts (A66 through A102)."""
        found = 0
        for n in range(self.VERDICT_START, self.VERDICT_END + 1):
            if _find_verdict(n) is not None:
                found += 1
        assert found == 37, f"Expected 37 verdicts, found {found}"


# -------------------------------------------------------------------
# Class 3: Schema version progression
# -------------------------------------------------------------------
class TestA103SchemaProgression:
    def test_schema_1_44_in_cli(self):
        """Schema 1.44 must appear in cli.py source."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.44"' in cli or '_AUDIT_SCHEMA_VERSION = "1.45"' in cli or '_AUDIT_SCHEMA_VERSION = "1.46"' in cli or '_AUDIT_SCHEMA_VERSION = "1.47"' in cli or '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or '_AUDIT_SCHEMA_VERSION = "1.49"' in cli or '_AUDIT_SCHEMA_VERSION = "1.50"' in cli or '_AUDIT_SCHEMA_VERSION = "1.51"' in cli or '_AUDIT_SCHEMA_VERSION = "1.52"' in cli or '_AUDIT_SCHEMA_VERSION = "1.53"' in cli or '_AUDIT_SCHEMA_VERSION = "1.54"' in cli or '_AUDIT_SCHEMA_VERSION = "1.55"' in cli or '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or '_AUDIT_SCHEMA_VERSION = "1.57"' in cli or '_AUDIT_SCHEMA_VERSION = "1.58"' in cli or '_AUDIT_SCHEMA_VERSION = "1.59"' in cli or '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or '_AUDIT_SCHEMA_VERSION = "1.61"' in cli

    def test_a102_contract_preserved(self):
        """A102 contract comment must still be in cli.py."""
        cli = _read_cli_source()
        assert "A102" in cli
    ...
    def test_all_contracts_a96_a103_present(self):
        """All contract comments A96-A103 must be in cli.py."""
        cli = _read_cli_source()
        for a_num in [96, 97, 98, 99, 100, 101, 102, 103]:
...
# -------------------------------------------------------------------
class TestA103TestInventory:
    def test_total_test_files_at_least_86(self):
        """There should be at least 86 test_paper_a*.py files."""
        all_files = sorted(glob.glob(str(_PROJECT_ROOT / "tests" / "test_paper_a*.py")))
        assert len(all_files) >= 86, f"Expected >= 86 test files, found {len(all_files)}"

    def test_a103_test_file_exists(self):
        """This test file itself must exist."""
        test_path = _PROJECT_ROOT / "tests" / "test_paper_a103_cumulative_chain.py"
...
# -------------------------------------------------------------------
class TestA103EvidenceStructure:
    ...
    def test_evidence_bundle_hash_in_manifest(self):
        ...
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A103.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A103.json not yet generated")
    ...
    def test_validate_script_uses_find_root(self):
        """validate_a103.py must use _find_root for dynamic path resolution."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a103.py"
        if not val_path.exists():
            val_path = _PROJECT_ROOT / "validate_a103.py"
        if not val_path.exists():
            pytest.skip("validate_a103.py not found")

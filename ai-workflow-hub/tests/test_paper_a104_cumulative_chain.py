"""A104 -- Cumulative Acceptance Chain Validation.

Validates that the complete chain of verdicts A66-A103 is preserved,
that schema version progression is intact, and that the evidence pack
structure remains coherent across all 38 acceptances.
...
# -------------------------------------------------------------------
class TestA104SchemaVersion:
    def test_schema_version_is_1_45(self):
        """cli.py must define _AUDIT_SCHEMA_VERSION = '1.45' exactly."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.45"' in cli or \
            ...
            "Schema version must be exactly 1.45 for A104"
        )

    def test_a104_contract_in_cli(self):
        """cli.py must contain A104 contract comment."""
        cli = _read_cli_source()
        assert "A104" in cli, "A104 contract comment must be present"

    def test_schema_forward_compat(self):
        """Schema must include 1.45 or 1.44."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.45"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.61"' in cli, (
            "Schema must include current version 1.45 or predecessor 1.44"
        )


# -------------------------------------------------------------------
# Class 2: Verdict chain completeness (A66-A103)
# -------------------------------------------------------------------
class TestA104VerdictChain:
    VERDICT_START = 66
    VERDICT_END = 103

    def test_all_verdict_files_present(self):
        """All verdict files A66-A103 must exist."""
    ...
    def test_verdict_count_is_38(self):
        """There should be exactly 38 verdicts (A66 through A103)."""
        found = 0
        for n in range(self.VERDICT_START, self.VERDICT_END + 1):
            if _find_verdict(n) is not None:
                found += 1
        assert found == 38, f"Expected 38 verdicts, found {found}"


# -------------------------------------------------------------------
# Class 3: Schema version progression
# -------------------------------------------------------------------
class TestA104SchemaProgression:
    def test_schema_1_45_in_cli(self):
        """Schema 1.45 must appear in cli.py source."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.45"' in cli or '_AUDIT_SCHEMA_VERSION = "1.46"' in cli or '_AUDIT_SCHEMA_VERSION = "1.47"' in cli or '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or '_AUDIT_SCHEMA_VERSION = "1.49"' in cli or '_AUDIT_SCHEMA_VERSION = "1.50"' in cli or '_AUDIT_SCHEMA_VERSION = "1.51"' in cli or '_AUDIT_SCHEMA_VERSION = "1.52"' in cli or '_AUDIT_SCHEMA_VERSION = "1.53"' in cli or '_AUDIT_SCHEMA_VERSION = "1.54"' in cli or '_AUDIT_SCHEMA_VERSION = "1.55"' in cli or '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or '_AUDIT_SCHEMA_VERSION = "1.57"' in cli or '_AUDIT_SCHEMA_VERSION = "1.58"' in cli or '_AUDIT_SCHEMA_VERSION = "1.59"' in cli or '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or '_AUDIT_SCHEMA_VERSION = "1.61"' in cli
    ...
    def test_all_contracts_a96_a104_present(self):
        """All contract comments A96-A104 must be in cli.py."""
        cli = _read_cli_source()
        for a_num in [96, 97, 98, 99, 100, 101, 102, 103, 104]:
...
# -------------------------------------------------------------------
class TestA104TestInventory:
    def test_total_test_files_at_least_87(self):
        """There should be at least 87 test_paper_a*.py files."""
        all_files = sorted(glob.glob(str(_PROJECT_ROOT / "tests" / "test_paper_a*.py")))
        assert len(all_files) >= 87, f"Expected >= 87 test files, found {len(all_files)}"

    def test_a104_test_file_exists(self):
        """This test file itself must exist."""
        test_path = _PROJECT_ROOT / "tests" / "test_paper_a104_cumulative_chain.py"
...
# -------------------------------------------------------------------
class TestA104EvidenceStructure:
    ...
    def test_evidence_bundle_hash_in_manifest(self):
        ...
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A104.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A104.json not yet generated")
    ...
    def test_validate_script_uses_find_root(self):
        """validate_a104.py must use _find_root for dynamic path resolution."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a104.py"
        if not val_path.exists():
            val_path = _PROJECT_ROOT / "validate_a104.py"
        if not val_path.exists():
            pytest.skip("validate_a104.py not found")

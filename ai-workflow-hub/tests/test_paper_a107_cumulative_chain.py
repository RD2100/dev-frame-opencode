"""A107 -- Cumulative Acceptance Chain Validation.

Validates that the complete chain of verdicts A66-A106 is preserved,
that schema version progression is intact, and that the evidence pack
structure remains coherent across all 41 acceptances.
...
# -------------------------------------------------------------------
class TestA107SchemaVersion:
    def test_schema_version_is_1_46(self):
        """cli.py must define _AUDIT_SCHEMA_VERSION = '1.48' exactly."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or \
            ...
            "Schema version must be exactly 1.48 for A107"
        )

    def test_a107_contract_in_cli(self):
        """cli.py must contain A107 contract comment."""
        cli = _read_cli_source()
        assert "A107" in cli, "A107 contract comment must be present"

    def test_schema_forward_compat(self):
        """Schema must include 1.48 or 1.47."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.61"' in cli, (
            "Schema must include current version 1.48 or predecessor 1.47"
        )


# -------------------------------------------------------------------
# Class 2: Verdict chain completeness (A66-A106)
# -------------------------------------------------------------------
class TestA107VerdictChain:
    VERDICT_START = 66
    VERDICT_END = 106

    def test_all_verdict_files_present(self):
        """All verdict files A66-A106 must exist."""
    ...
    def test_verdict_count_is_41(self):
        """There should be exactly 41 verdicts (A66 through A106)."""
        found = 0
        for n in range(self.VERDICT_START, self.VERDICT_END + 1):
            if _find_verdict(n) is not None:
                found += 1
        assert found == 41, f"Expected 41 verdicts, found {found}"


# -------------------------------------------------------------------
# Class 3: Schema version progression
# -------------------------------------------------------------------
class TestA107SchemaProgression:
    def test_schema_1_46_in_cli(self):
        """Schema 1.48 must appear in cli.py source."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or '_AUDIT_SCHEMA_VERSION = "1.49"' in cli or '_AUDIT_SCHEMA_VERSION = "1.50"' in cli or '_AUDIT_SCHEMA_VERSION = "1.51"' in cli or '_AUDIT_SCHEMA_VERSION = "1.52"' in cli or '_AUDIT_SCHEMA_VERSION = "1.53"' in cli or '_AUDIT_SCHEMA_VERSION = "1.54"' in cli or '_AUDIT_SCHEMA_VERSION = "1.55"' in cli or '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or '_AUDIT_SCHEMA_VERSION = "1.57"' in cli or '_AUDIT_SCHEMA_VERSION = "1.58"' in cli or '_AUDIT_SCHEMA_VERSION = "1.59"' in cli or '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or '_AUDIT_SCHEMA_VERSION = "1.61"' in cli
    ...
    def test_all_contracts_a96_a107_present(self):
        """All contract comments A96-A107 must be in cli.py."""
        cli = _read_cli_source()
        for a_num in [96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107]:
...
# -------------------------------------------------------------------
class TestA107TestInventory:
    def test_total_test_files_at_least_90(self):
        """There should be at least 90 test_paper_a*.py files."""
        all_files = sorted(glob.glob(str(_PROJECT_ROOT / "tests" / "test_paper_a*.py")))
        assert len(all_files) >= 90, f"Expected >= 90 test files, found {len(all_files)}"

    def test_a107_test_file_exists(self):
        """This test file itself must exist."""
        test_path = _PROJECT_ROOT / "tests" / "test_paper_a107_cumulative_chain.py"
...
# -------------------------------------------------------------------
class TestA107EvidenceStructure:
    ...
    def test_evidence_bundle_hash_in_manifest(self):
        ...
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A107.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A107.json not yet generated")
    ...
    def test_validate_script_uses_find_root(self):
        """validate_a107.py must use _find_root for dynamic path resolution."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a107.py"
        if not val_path.exists():
            val_path = _PROJECT_ROOT / "validate_a107.py"
        if not val_path.exists():
            pytest.skip("validate_a107.py not found")

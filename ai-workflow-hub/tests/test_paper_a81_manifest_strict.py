"""A81 -- Strict Manifest Cross-Check (Fail-Closed).

From A80 accepted directive:
  Make count-manifest validation fail-closed. Cross-check ALL manifest
  counts against actual evidence files and transcripts. Mismatches are
  FAILURES, not warnings.

Fix:
  1. Cross-check ALL 6 manifest keys against evidence (not just 2).
  2. ALL mismatches are FAILURES (errors += 1), not warnings.
  3. Schema version bumped to 1.22 (accepts 1.20/1.21/1.22).
  4. Platform field added to manifest.

Verifies:
1. Schema version "1.22" (with OR chain for backwards compat).
2. COUNTS_MANIFEST_A81.json exists and contains required keys.
3. ALL 6 manifest keys cross-checked against actual evidence.
4. Mismatches are failures, not warnings (fail-closed).
5. All A81 invariants preserved from A75-A80.
6. Regression safety.

CDP directive (from A80 verdict):
  "Make count-manifest validation fail-closed. Cross-check ALL manifest
   counts against actual evidence files and transcripts. Mismatches are
   FAILURES, not warnings. Schema 1.22."
"""
...
# -------------------------------------------------------------------
# Class 1: Schema version 1.22
# -------------------------------------------------------------------
class TestA81SchemaVersion:
    def test_schema_version_is_1_22(self):
        """Schema must be 1.20, 1.21, or 1.22 (OR chain for compat)."""
        cli = _read_cli_source()
        assert (
            '_AUDIT_SCHEMA_VERSION = "1.20"' in cli
        ...
        ), "Schema version must be 1.20, 1.21, 1.22, or 1.23"
    ...
    def test_schema_version_in_output(self, tmp_path):
        ...
        if r.exit_code == 0:
            ...
            assert data.get("result_schema_version") in ("1.20", "1.21", "1.22", "1.23", "1.24", "1.25", "1.26", "1.27", "1.28", "1.29", "1.30", "1.31", "1.32", "1.33", "1.34", "1.35", "1.36", "1.37", "1.38", "1.39", "1.40", "1.41", "1.42", "1.43", "1.44", "1.45", "1.46")

    def test_schema_122_accepted(self):
        """Schema 1.22 must be in the accepted OR chain."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.22"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.39"' in cli or \
                   ...
                   '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
                   '_AUDIT_SCHEMA_VERSION = "1.61"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.21"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.20"' in cli, \
            "Schema OR chain must include 1.20-1.39"


# -------------------------------------------------------------------
# Class 2: Manifest strict cross-check (fail-closed)
# -------------------------------------------------------------------
class TestA81ManifestStrict:
    _REQUIRED_KEYS = (
        "total_test_files",
        "in_scope",
        "out_of_scope",
    ...
    def _load_manifest(self) -> dict:
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A81.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A81.json not yet generated")
        return json.loads(manifest_path.read_text(encoding="utf-8"))

    def test_manifest_exists(self):
        """COUNTS_MANIFEST_A81.json must exist at project root."""
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A81.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A81.json not yet generated (pack script creates it)")
    ...
    def test_manifest_total_test_files_matches_actual(self):
        ...
        assert actual_count >= manifest_count, (
            f"FAIL-CLOSED: Manifest total_test_files={manifest_count} but actual "
            f"glob count={actual_count} (fewer files than manifest?)"
        )

    def test_manifest_new_tests_matches_actual(self):
        """Manifest new_tests must match actual test count in A81 test file (fail-closed)."""
        data = self._load_manifest()
        a81_test_path = _PROJECT_ROOT / "tests" / "test_paper_a81_manifest_strict.py"
        if not a81_test_path.exists():
            pytest.skip("test_paper_a81_manifest_strict.py not found")
        a81_src = a81_test_path.read_text(encoding="utf-8")
        actual_count = len(re.findall(r"^\s+def test_", a81_src, re.MULTILINE))
        manifest_count = data.get("new_tests", 0)
        assert manifest_count == actual_count, (
            f"FAIL-CLOSED: Manifest new_tests={manifest_count} but actual "
            f"test count in A81 file={actual_count}"
        )

    def test_manifest_regression_passed_matches_output(self):
        """Manifest regression_passed must match REGRESSION_OUTPUT text (fail-closed)."""
        data = self._load_manifest()
        reg_path = _PROJECT_ROOT / "REGRESSION_OUTPUT_A81.txt"
        if not reg_path.exists():
            pytest.skip("REGRESSION_OUTPUT_A81.txt not yet generated")
        reg_text = reg_path.read_text(encoding="utf-8")
        match = re.search(r"(\d+)\s+passed", reg_text)
        if match:
            actual_passed = int(match.group(1))
            manifest_passed = data.get("regression_passed", 0)
            assert manifest_passed == actual_passed, (
                f"FAIL-CLOSED: Manifest regression_passed={manifest_passed} but "
                f"REGRESSION_OUTPUT shows {actual_passed} passed"
            )
...
# -------------------------------------------------------------------
class TestA81Invariants:
    def test_a81_contract_in_cli(self):
        """cli.py must contain A81 contract comment."""
        cli = _read_cli_source()
        assert "A81" in cli, "A81 contract comment missing from cli.py"
    ...
    def test_bootstrap_in_validate(self):
        """validate_a81.py must include dependency bootstrap."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a81.py"
        if not val_path.exists():
            pytest.skip("validate_a81.py not found")
        val_src = val_path.read_text(encoding="utf-8")
        assert "pip install" in val_src or "bootstrap" in val_src.lower(), (
            "validate_a81.py must include dependency bootstrap"
        )

    def test_manifest_crosscheck_is_fail_closed(self):
        """validate_a81.py cross-check mismatches must be failures, not warnings."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a81.py"
        if not val_path.exists():
            pytest.skip("validate_a81.py not found")
        val_src = val_path.read_text(encoding="utf-8")
        # In cross-check sections, WARN must NOT appear; only FAIL/errors += 1
        in_crosscheck = False
        warn_lines = []
        for i, line in enumerate(val_src.splitlines(), 1):
            if "Cross-Check" in line or "cross-check" in line.lower():
                in_crosscheck = True
            if in_crosscheck and "WARN" in line:
                warn_lines.append((i, line.strip()))
        assert len(warn_lines) == 0, (
            f"FAIL-CLOSED violation: found WARN in cross-check sections: {warn_lines}"
        )
...
class TestA81RegressionSafety:
    ...
    def test_prompt_exists(self):
        """GPT_REVIEW_PROMPT_A81.txt must exist."""
        prompt_path = _PROJECT_ROOT / "scripts" / "GPT_REVIEW_PROMPT_A81.txt"
        if not prompt_path.exists():
            pytest.skip("GPT_REVIEW_PROMPT_A81.txt not in scope")
        assert prompt_path.exists(), (
            "GPT_REVIEW_PROMPT_A81.txt must exist in scripts/"
        )

"""A115 -- Cumulative Acceptance Chain Validation.

Validates that the complete chain of verdicts A66-A114 is preserved,
that schema version progression is intact, and that the evidence pack
structure remains coherent across all 49 acceptances.
...
# -------------------------------------------------------------------
class TestA115SchemaVersion:
    def test_schema_version_is_1_54(self):
        """cli.py must define _AUDIT_SCHEMA_VERSION = '1.56' exactly."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or \
            ...
            "Schema version must be exactly 1.56 for A115"
        )

    def test_a115_contract_in_cli(self):
        """cli.py must contain A115 contract comment."""
        cli = _read_cli_source()
        assert "A115" in cli, "A115 contract comment must be present"

    def test_schema_forward_compat(self):
        """Schema must include 1.56 or 1.55."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.61"' in cli, (
            "Schema must include current version 1.56 or predecessor 1.55"
        )


# -------------------------------------------------------------------
# Class 2: Verdict chain completeness (A66-A114)
# -------------------------------------------------------------------
class TestA115VerdictChain:
    VERDICT_START = 66
    VERDICT_END = 114

    def test_all_verdict_files_present(self):
        """All verdict files A66-A114 must exist."""
    ...
    def test_verdict_count_is_49(self):
        """There should be exactly 49 verdicts (A66 through A114)."""
        found = 0
        for n in range(self.VERDICT_START, self.VERDICT_END + 1):
            if _find_verdict(n) is not None:
                found += 1
        assert found == 49, f"Expected 49 verdicts, found {found}"


# -------------------------------------------------------------------
# Class 3: Schema version progression
# -------------------------------------------------------------------
class TestA115SchemaProgression:
    def test_schema_1_54_in_cli(self):
        """Schema 1.56 must appear in cli.py source."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or '_AUDIT_SCHEMA_VERSION = "1.57"' in cli or '_AUDIT_SCHEMA_VERSION = "1.58"' in cli or '_AUDIT_SCHEMA_VERSION = "1.59"' in cli or '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or '_AUDIT_SCHEMA_VERSION = "1.61"' in cli
    ...
    def test_all_contracts_a96_a115_present(self):
        """All contract comments A96-A115 must be in cli.py."""
        cli = _read_cli_source()
        for a_num in [96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115]:
...
# -------------------------------------------------------------------
class TestA115TestInventory:
    def test_total_test_files_at_least_98(self):
        """There should be at least 98 test_paper_a*.py files."""
        all_files = sorted(glob.glob(str(_PROJECT_ROOT / "tests" / "test_paper_a*.py")))
        assert len(all_files) >= 98, f"Expected >= 98 test files, found {len(all_files)}"

    def test_a115_test_file_exists(self):
        """This test file itself must exist."""
        test_path = _PROJECT_ROOT / "tests" / "test_paper_a115_cumulative_chain.py"
...
# -------------------------------------------------------------------
class TestA115EvidenceStructure:
    ...
    def test_evidence_bundle_hash_in_manifest(self):
        ...
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A115.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A115.json not yet generated")
    ...
    def test_validate_script_uses_find_root(self):
        """validate_a115.py must use _find_root for dynamic path resolution."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a115.py"
        if not val_path.exists():
            val_path = _PROJECT_ROOT / "validate_a115.py"
        if not val_path.exists():
            pytest.skip("validate_a115.py not found")

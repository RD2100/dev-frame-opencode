"""A106 -- Cumulative Acceptance Chain Validation.

Validates that the complete chain of verdicts A66-A105 is preserved,
that schema version progression is intact, and that the evidence pack
structure remains coherent across all 40 acceptances.
...
# -------------------------------------------------------------------
class TestA106SchemaVersion:
    def test_schema_version_is_1_46(self):
        """cli.py must define _AUDIT_SCHEMA_VERSION = '1.47' exactly."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.47"' in cli or \
            ...
            "Schema version must be exactly 1.47 for A106"
        )

    def test_a106_contract_in_cli(self):
        """cli.py must contain A106 contract comment."""
        cli = _read_cli_source()
        assert "A106" in cli, "A106 contract comment must be present"

    def test_schema_forward_compat(self):
        """Schema must include 1.47 or 1.46."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.47"' in cli or \
               ...
               '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or \
               '_AUDIT_SCHEMA_VERSION = "1.61"' in cli, (
            "Schema must include current version 1.47 or predecessor 1.46"
        )


# -------------------------------------------------------------------
# Class 2: Verdict chain completeness (A66-A105)
# -------------------------------------------------------------------
class TestA106VerdictChain:
    VERDICT_START = 66
    VERDICT_END = 105

    def test_all_verdict_files_present(self):
        """All verdict files A66-A105 must exist."""
    ...
    def test_verdict_count_is_40(self):
        """There should be exactly 40 verdicts (A66 through A105)."""
        found = 0
        for n in range(self.VERDICT_START, self.VERDICT_END + 1):
            if _find_verdict(n) is not None:
                found += 1
        assert found == 40, f"Expected 40 verdicts, found {found}"


# -------------------------------------------------------------------
# Class 3: Schema version progression
# -------------------------------------------------------------------
class TestA106SchemaProgression:
    def test_schema_1_46_in_cli(self):
        """Schema 1.47 must appear in cli.py source."""
        cli = _read_cli_source()
        assert '_AUDIT_SCHEMA_VERSION = "1.47"' in cli or '_AUDIT_SCHEMA_VERSION = "1.48"' in cli or '_AUDIT_SCHEMA_VERSION = "1.49"' in cli or '_AUDIT_SCHEMA_VERSION = "1.50"' in cli or '_AUDIT_SCHEMA_VERSION = "1.51"' in cli or '_AUDIT_SCHEMA_VERSION = "1.52"' in cli or '_AUDIT_SCHEMA_VERSION = "1.53"' in cli or '_AUDIT_SCHEMA_VERSION = "1.54"' in cli or '_AUDIT_SCHEMA_VERSION = "1.55"' in cli or '_AUDIT_SCHEMA_VERSION = "1.56"' in cli or '_AUDIT_SCHEMA_VERSION = "1.57"' in cli or '_AUDIT_SCHEMA_VERSION = "1.58"' in cli or '_AUDIT_SCHEMA_VERSION = "1.59"' in cli or '_AUDIT_SCHEMA_VERSION = "1.60"' in cli or '_AUDIT_SCHEMA_VERSION = "1.61"' in cli
    ...
    def test_all_contracts_a96_a106_present(self):
        """All contract comments A96-A106 must be in cli.py."""
        cli = _read_cli_source()
        for a_num in [96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106]:
...
# -------------------------------------------------------------------
class TestA106TestInventory:
    def test_total_test_files_at_least_89(self):
        """There should be at least 89 test_paper_a*.py files."""
        all_files = sorted(glob.glob(str(_PROJECT_ROOT / "tests" / "test_paper_a*.py")))
        assert len(all_files) >= 89, f"Expected >= 89 test files, found {len(all_files)}"

    def test_a106_test_file_exists(self):
        """This test file itself must exist."""
        test_path = _PROJECT_ROOT / "tests" / "test_paper_a106_cumulative_chain.py"
...
# -------------------------------------------------------------------
class TestA106EvidenceStructure:
    ...
    def test_evidence_bundle_hash_in_manifest(self):
        ...
        manifest_path = _PROJECT_ROOT / "COUNTS_MANIFEST_A106.json"
        if not manifest_path.exists():
            pytest.skip("COUNTS_MANIFEST_A106.json not yet generated")
    ...
    def test_validate_script_uses_find_root(self):
        """validate_a106.py must use _find_root for dynamic path resolution."""
        val_path = _PROJECT_ROOT / "scripts" / "validate_a106.py"
        if not val_path.exists():
            val_path = _PROJECT_ROOT / "validate_a106.py"
        if not val_path.exists():
            pytest.skip("validate_a106.py not found")
